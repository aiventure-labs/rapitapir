...
module RapiTapir
  ...
  def self.load_rails_integration!
    ...
    if RapiTapir::Server::Rails.const_defined?(:ControllerBase)
    ...
    if defined?(Rails::Railtie) && !Object.const_defined?(:RapiTapirRailtie)
      Object.const_set(:RapiTapirRailtie, Class.new(Rails::Railtie) do
        initializer "rapitapir.extend_routes", after: :initialize_routes do |app|
          app.routes.extend(RapiTapir::Server::Rails::Routes) if app.routes
        end
      end)
    end
      ...
      module Rails
        class ControllerBase
          def self.method_missing(method, *args, &block)
            ...
            RapiTapir.load_rails_integration!
            if defined?(RapiTapir::Server::Rails::ControllerBase) && RapiTapir::Server::Rails::ControllerBase != self.class
              return RapiTapir::Server::Rails::ControllerBase.send(method, *args, &block)
            end
            raise NameError, "Rails integration not available. Please ensure Rails is loaded before requiring RapiTapir."
          end
        end
      end
    end
